...
class NotificationServiceImpl @Inject constructor(
    ...
    override suspend fun createUserNotification(
        ...
        return try {
            ...
            Log.e("NotificationService", "Error creating user notification: ${e.message}", e)
            Result.failure(e)
        }
    }
    ...
    override suspend fun createDoctorNotification(
        ...
        return try {
            ...
            Log.e("NotificationService", "Error creating doctor notification: ${e.message}")
            Result.failure(e)
        }
    }
            ...
            override fun onDataChange(snapshot: DataSnapshot) {
                try {
                    val notifications = snapshot.children.mapNotNull { notificationSnapshot ->
                        try {
                            Notification(
                                id = notificationSnapshot.child("id").getValue(String::class.java) ?: "",
                                title = notificationSnapshot.child("title").getValue(String::class.java) ?: "",
                                message = notificationSnapshot.child("message").getValue(String::class.java) ?: "",
                                time = notificationSnapshot.child("time").getValue(Long::class.java) ?: 0L,
                                read = notificationSnapshot.child("read").getValue(Boolean::class.java) ?: false,
                                type = try {
                                    val typeString = notificationSnapshot.child("type").getValue(String::class.java) ?: "SYSTEM"
                                    NotificationType.valueOf(typeString)
                                } catch (e: Exception) {
                                    NotificationType.SYSTEM
                                },
                                date = notificationSnapshot.child("date").getValue(String::class.java) ?: "",
                                appointmentId = notificationSnapshot.child("appointmentId").getValue(String::class.java) ?: ""
                            )
                        } catch (e: Exception) {
    ...
    override suspend fun markAsRead(notificationId: String, userId: String): Result<Unit> {
        return try {
            val notificationRef = database.getReference("notifications/users/$userId/$notificationId")
